...
in_dir =  "/SABER_instrument/2002/001"#SABER_L2A_2002026_00736_02.0.nc"
...
HERE THE WORK DONE IN THE SCRIPT Saber_single.py is added. It is better to first have a full understanding of a 
...
and PHI is the phase, defined as the time of maximum amplitude in s.
...
We ignore err here and I give as X the time (in our case 24 s with a step of 1 ) and Y the the meridional wind at each time (this is done at every model grid-point). The fit returns p[0] and p[1] that minimize the least-squares. 
...
for j in range(4): # Planetary wave period
    for s in range(-3,4): # Wavenumber: <0 westward propagation, >0 eastward propagation
        amplitude_A * np.cos(((2*np.pi)/(24*T[j]))* - ((2*np.pi)/360)* s * np.deg2rad(longitude) - ((2*np.pi/360)*phaseA))
...
for k in range(1, 3):# number of diurnal harmonic 1-2
    for s in range(-4, 5): # Wavenumber: <0 westward propagation, >0 eastward propagation
        amplitude_C * np.cos(k*(2*np.pi/24)* - (2*np.pi/360)* s * np.deg2rad(longitude) - (2*np.pi/360)*phaseC) 
